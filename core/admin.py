...
class CustomUserAdmin(BaseUserAdmin):
    """Extended User admin with profile inline and role management."""

    inlines = [UserProfileInline]
    list_display = ['avatar_preview', 'username', 'email', 'full_name', 'role_badge', 'is_active', 'last_login', 'date_joined']
    ...
    def avatar_preview(self, obj):
        if hasattr(obj, 'profile') and obj.profile.avatar:
            return format_html(
                '<img src="{}" width="40" height="40" style="object-fit:cover;border-radius:50%;border:2px solid #1e2a4a;"/>',
                obj.profile.avatar.url
            )
        # Default avatar with initials
        initials = f"{obj.first_name[:1]}{obj.last_name[:1]}".upper() if obj.first_name and obj.last_name else "?"
        is_super = is_user_super_admin(obj)
        color = '#1e2a4a' if is_super else '#4caf50'
        return format_html(
            '<div style="width:40px;height:40px;border-radius:50%;background:{};color:white;display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:14px;">{}</div>',
            color, initials
        )
    ...
    def role_badge(self, obj):
        ...
        if is_super:
            return format_html(
                '<span style="background:linear-gradient(135deg, #1e2a4a 0%, #2d3e5f 100%);color:white;padding:4px 12px;border-radius:15px;font-size:11px;font-weight:600;">'
                '👑 Super Admin</span>'
            )
        return format_html(
            '<span style="background:linear-gradient(135deg, #4caf50 0%, #8bc34a 100%);color:white;padding:4px 12px;border-radius:15px;font-size:11px;font-weight:600;">'
            '👤 Admin</span>'
        )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        # Only count staff users (admin users)
        staff_users = User.objects.filter(is_staff=True)
        total = staff_users.count()
        super_admins = staff_users.filter(is_superuser=True).count()
        # Count profiles with admin role
        admin_profiles = UserProfile.objects.filter(role='admin').count()
        active = staff_users.filter(is_active=True).count()
        inactive = staff_users.filter(is_active=False).count()

        # Recent logins
        week_ago = timezone.now() - timedelta(days=7)
        recent_logins = staff_users.filter(last_login__gte=week_ago).count()
    ...
    def get_queryset(self, request):
        ...
        # Simple admins can only see themselves
        if not is_user_super_admin(request.user):
            return qs.filter(pk=request.user.pk)
        return qs

    def has_add_permission(self, request):
        # Only super admins can add users
        return is_user_super_admin(request.user)

    def has_change_permission(self, request, obj=None):
        if obj is None:
            return True
        # Super admins can edit everyone
        if is_user_super_admin(request.user):
            return True
        # Simple admins can only edit themselves
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        # Simple admins cannot change permissions
        if not is_user_super_admin(request.user):
...
# Mixin pour restreindre les permissions des admins simples
class RoleBasedAdminMixin:
    """Mixin to restrict access based on user role."""

    # Models that simple admins cannot access
    restricted_for_simple_admin = False

    def has_module_permission(self, request):
        if self.restricted_for_simple_admin:
            if not is_user_super_admin(request.user):
                return False
